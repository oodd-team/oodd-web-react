...
import { GetPostListData } from "../../apis/post/dto";

const MyPage: React.FC = () => {
    const [isLoading, setIsLoading] = useState(true);
    const [isBottomSheetOpen, setIsBottomSheetOpen] = useState(false);
    const [posts, setPosts] = useState<GetPostListData['post']>([]);
    ...
    const handleOpenSheet = () => {
		...
		const fetchPostList = async () => {
			try {
					...
					// API 호출
					const response = await getPostListApi(1, 10, Number(storedUserId));
            ...
            <ProfileContainer>
                ...
                <StatsContainer>
                    ...
                    <Stat>
                        ...
                        <StatNumber>{posts.reduce((sum, post) => sum + post.commentCount, 0)}</StatNumber>
                    </Stat>
                    <Stat>
                        <StatLabel>좋아요</StatLabel>
                        <StatNumber>{posts.reduce((sum, post) => sum + post.likeCount, 0)}</StatNumber>
                    </Stat>
                </StatsContainer>
                <PostsContainer>
                    {posts.length > 0 ? (
